...
unitlist size  = 
  if size == 9 then
    ...
    [cross rows [c] | c <- smallCols]
    ++ [cross [r] cols | r <- smallRows]
    ++ [cross rs cs | rs <- smallRowBoxes, cs <- smallColBoxes]

units :: Int -> [(String, [[String]])]
units size = [(s, filter (elem s) (unitlist size)) | s <- squares]
...
prettyPrint board size = do
  let zipped = zip board (map snd ((validBoardNumbers (parseBoardToString board size)) size ))
  let units =  [(x, validUnit x (validBoardNumbers (parseBoardToString board size) size )) | x <- unitList size]
  if validUnits (parseBoardToString board size) size then
    ...
    --map (\x -> if not validUnit x then "Fel i unit" + fst x else "Inte fel i unit" + x) parseBoardToString board
...
parseBoardToString :: String -> Int -> [(String, Int)]
parseBoardToString a size = zip (dynamicCross size) (map digitToInt (replacePointsWithZeros a))
...
validSquareNumbers1 :: (String, Int) -> [(String, Int)] -> Int-> (String, [Int])
validSquareNumbers1 x ys size = (fst x, reduceList ints (lookups (getPeers (fst x) size) ys))
...
validUnits y size = and [validUnit x (validBoardNumbers y size) | x <- unitList size]

--Given input String (i.e 12....34.......) returns if it is possible to solve sudoku
verifySudoku :: String -> Bool
verifySudoku x  =
  if length x == 81 then
    validUnits (parseBoardToString x 9) 9
  else 
    validUnits (parseBoardToString x 4) 4
